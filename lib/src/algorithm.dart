...
AStarNode<T>? aStar<T extends AStarState<T>>(
  T state, {
  bool verbose = false,
  int limit = 1000,
}) {
  ...
  while (open.isNotEmpty) {
    ...
    if (verbose) {
      print(
        "[$count] Exploring: ${node.hash} (${node.depth} + ${node.heuristic} = ${node.cost})",
      );
    }
    ...
    for (final newNode in node.expand()) {
      ...
      if (verbose) {
        print("[$count]   Got: ${newNode.hash} (cost = ${newNode.heuristic})");
      }
      open.add(newNode);
      opened.add(newNode);
    }
